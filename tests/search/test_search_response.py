...
class MockSearchResult:
    def __init__(self, message_id, content_snippet, created_at, rank):
...
class TestSearchResponse(unittest.TestCase):
    def test_init_with_results_and_total_count(self):
        ...
        # given
        mock_result1 = MockSearchResult(uuid.uuid4(), "Snippet 1", datetime.now(), 0.9)
        mock_result2 = MockSearchResult(uuid.uuid4(), "Snippet 2", datetime.now(), 0.8)
    ...
    def test_init_with_results_and_default_total_count(self):
        ...
        # given
        mock_result = MockSearchResult(uuid.uuid4(), "Snippet", datetime.now(), 0.7)
        results = [mock_result]
    ...
    def test_has_results_when_results_not_empty(self):
        ...
        # given
        mock_result = MockSearchResult(uuid.uuid4(), "Snippet", datetime.now(), 0.6)
        results = [mock_result]
        response = SearchResponse(results=results)
